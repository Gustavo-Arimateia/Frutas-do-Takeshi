...
import './CadastrarFruta.css'
import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { v4 as uuidv4 } from 'uuid';


export const CadastrarFruta = () => {
    ...
    const PegarValor = (value, labelName) => {
    ...
    const BotaoClicado = () => {
        ...
        const fruta = {
            id: uuidv4(),
            Nome: labelValues.Nome,
            Preco: labelValues.Preco,
            Estoque: labelValues.Estoque
        };

        const frutasCadastradas = JSON.parse(localStorage.getItem('frutasCadastradasTeste3')) || [];
        frutasCadastradas.push(fruta);
        localStorage.setItem('frutasCadastradasTeste3', JSON.stringify(frutasCadastradas));
        Navigate('/FrutaCadastrada');
    };
    ...
    return (
        <>
            <div className="header">
                ...
                >
                    <path
                        d="M18.1213 16L24.0588 10.0625C24.3405 9.7812 24.4991 9.39948 24.4994 9.00132C24.4998 8.60316 24.3419 8.22116 24.0606 7.93937C23.7793 7.65758 23.3976 7.49907 22.9995 7.49872C22.6013 7.49837 22.2193 7.6562 21.9375 7.93749L16 13.875L10.0625 7.93749C9.78072 7.6557 9.39852 7.49739 9.00001 7.49739C8.60149 7.49739 8.2193 7.6557 7.93751 7.93749C7.65572 8.21929 7.49741 8.60148 7.49741 8.99999C7.49741 9.39851 7.65572 9.7807 7.93751 10.0625L13.875 16L7.93751 21.9375C7.65572 22.2193 7.49741 22.6015 7.49741 23C7.49741 23.3985 7.65572 23.7807 7.93751 24.0625C8.2193 24.3443 8.60149 24.5026 9.00001 24.5026C9.39852 24.5026 9.78072 24.3443 10.0625 24.0625L16 18.125L21.9375 24.0625C22.2193 24.3443 22.6015 24.5026 23 24.5026C23.3985 24.5026 23.7807 24.3443 24.0625 24.0625C24.3443 23.7807 24.5026 23.3985 24.5026 23C24.5026 22.6015 24.3443 22.2193 24.0625 21.9375L18.1213 16Z"
                        fill="#212324"
                    />
                </svg>
            </div>
            <div className="Label">
            <Label className='Label' icon={iconMaça} typeValor="text" onChange={value => PegarValor(value, 'Nome')}>
                Nome da Fruta
            </Label>
            </div>
            <div className="Label">
            <Label className='Label' icon={iconDinheiro} typeValor="number" onChange={value => PegarValor(value, 'Preco')}>
                Preço do Kilo
            </Label>
            </div>
            <div className="Label">
            <Label className='Label'icon={iconMoedas} typeValor="number" onChange={value => PegarValor(value, 'Estoque')}>
